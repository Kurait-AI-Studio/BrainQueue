...
export function Dim({ label, value, onChange }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "0.25rem" }}>
      <span style={{ fontSize: "0.62rem", color: "#555", fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", width: "14px" }}>{label}</span>
  ...
  return (
    <div style={{ marginBottom: "1.2rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.4rem" }}>
        <div>
          <span style={{ fontSize: "0.78rem", color: "#aaa", fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", fontWeight: 700 }}>{label}</span>
          <span style={{ fontSize: "0.68rem", color: "#444", marginLeft: "0.5rem" }}>{description}</span>
        </div>
        <span style={{ fontSize: "0.8rem", color: "#bef24a", fontWeight: 700, fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", minWidth: "32px", textAlign: "right" }}>{value}</span>
      </div>
...
export function EmptyState({ view, filterCat, onAdd, onDump }) {
  ...
  return (
    <div style={{ textAlign: "center", padding: "4.5rem 1rem", maxWidth: "400px", margin: "0 auto" }}>
      ...
      <h3 style={{ fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", fontWeight: 800, fontSize: "1.1rem", color: "#d8d8e0", margin: 0 }}>{c.title}</h3>
      ...
      {view !== 4 && (
        <div style={{ display: "flex", gap: "0.6rem", justifyContent: "center", marginTop: "1.5rem", flexWrap: "wrap" }}>
          <GlassButton onClick={onAdd} accent="#bef24a" style={{ padding: "0.6rem 1.1rem" }}>+ Add task</GlassButton>
  ...
  return (
    <span style={{ display: "inline-flex", gap: "0.25rem", alignItems: "center" }}>
      <input value={v} onChange={e => setV(e.target.value)} onKeyDown={e => { if (e.key === "Enter") add(); }}
        ...
        style={{ ...glass, borderRadius: "20px", padding: "0.26rem 0.7rem", color: "#e8e8e8", fontSize: "0.72rem", fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", outline: "none", width: "130px", boxSizing: "border-box" }} />
      {v.trim() && <button onClick={add} style={{ ...glass, borderRadius: "20px", padding: "0.26rem 0.6rem", color: "#bef24a", cursor: "pointer", fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", fontWeight: 700, fontSize: "0.72rem", border: "1px solid rgba(232,255,90,0.3)" }}>Add</button>}
    </span>
  );
}
...
export function Toast({ toast, onDone }) {
  ...
  return (
    <div onClick={onDone} style={{
      ...
      <span style={{ fontSize: "0.8rem", color: ok ? "#cfe" : "#fcc", fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>{toast.msg}</span>
    </div>
  );
}
  ...
  return (
    <div title={u.email || name} style={{
      ...
      {avatar
        ...
        : <span style={{ width: "22px", height: "22px", borderRadius: "50%", background: "rgba(232,255,90,0.18)", color: "#bef24a", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.7rem", fontWeight: 800, fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>{(name[0] || "?").toUpperCase()}</span>}
